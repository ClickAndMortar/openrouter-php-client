...
/**
 * Web-search server tool (2025-08-26 surface). All fields are optional —
 * defaults are decided by the upstream provider.
 */
final class WebSearchServerTool implements Tool
{
    /**
    ...
    public static function from(array $attributes): self
    {
        ...
        if (isset($attributes['engine']) && is_string($attributes['engine'])) {
            ...
            if ($engine === null) {
                throw new InvalidArgumentException(sprintf(
                    'WebSearchServerTool::$engine must be one of %s, got "%s"',
        ...
        if (isset($attributes['search_context_size']) && is_string($attributes['search_context_size'])) {
            ...
            if ($searchContextSize === null) {
                throw new InvalidArgumentException(sprintf(
                    'WebSearchServerTool::$searchContextSize must be one of %s, got "%s"',
