...
/**
 * Web-search preview server tool (`web_search_preview` surface). Mirrors the
 * `Preview_WebSearchServerTool` schema from the OpenRouter OpenAPI spec.
 */
final class WebSearchPreviewServerTool implements Tool
{
    /**
    ...
    public static function from(array $attributes): self
    {
        ...
        if (isset($attributes['engine']) && is_string($attributes['engine'])) {
            ...
            if ($engine === null) {
                throw new InvalidArgumentException(sprintf(
                    'WebSearchPreviewServerTool::$engine must be one of %s, got "%s"',
        ...
        if (isset($attributes['search_context_size']) && is_string($attributes['search_context_size'])) {
            ...
            if ($searchContextSize === null) {
                throw new InvalidArgumentException(sprintf(
                    'WebSearchPreviewServerTool::$searchContextSize must be one of %s, got "%s"',
